...
const
    doc = document,
    opened: { [id: string]: DialogOptions } = {};
...
export function open<T>(options: DialogOptions): Promise<T> {
    const
        opts = extend({}, defaults, options) as DialogOptions;
...
export function close(id: string, result?: any): Promise<void> {
...
function defaultClose(options: DialogOptions): void {
    const
        ...
        dialog = doc.getElementById(options.id);
...
function ensureGlobalContainer(container: string | Node): Node {
...
class TemplateViewModel {
    ...
    public activate(options: DialogOptions) {
        this.id = options.id;
        this._title = options.title;
    }

    public bindingComplete(container: HTMLElement, options: DialogOptions) {
